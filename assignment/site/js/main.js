    ...
    $.getJSON('content/products.json', function (data) {
        ...
        $.each(data, function (key, val) {
            ...
            var listItem = 
                '<li>' +
                    '<a href="#">' +
                        '<img style="padding: 20px 0 0 10px" src="content/assets/productImages/' + orderLineItem.product.image + '"/>' + 
...
function buildOptions(options) {
    ...
    $.each(options, function(size, price) {
        ...
        '<button class="Option-Button-Sizes" onclick="console.log(\'hello\')">' + size + '</button>&nbsp;&nbsp;'
    });
    ...
    $.each(orderLineItems, function (key, orderLineItem) {
        if (orderLineItem.count != 0) {
            ...
            var listSelectedItem = 
                '<li>' +
                    '<a href="#">' +
                        '<img style="padding: 7px 0 0 8px" src="content/assets/productImages/' + orderLineItem.product.image + '"/>' + 
                        '<h2>' + orderLineItem.product.name + '</h2>' +
